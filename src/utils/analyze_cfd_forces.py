...
def polar_plot(x_axis, y_axis, z_axis, forces, calado, rotacao, title, rmin=None, rmax=None, coef=True):
    ...
    for i, V in enumerate(y_axis):
        ...
        # plot linhas e preenchimento
        ax.plot(theta, z_plot, color=colors[i], linewidth=2, alpha=0.95, label=f"V = {V} m/s")
    ...
    fname = f"figures/coef_{forces}_calado_{calado}_rotacao_{rotacao}.png" if coef else f"figures/{forces}_calado_{calado}_rotacao_{rotacao}.png"
...
# polar_plot(x_axis, y_axis, case_1, 'fx_rotores', 8.5, 100, f"$C_x$ (rotores) para $T = ${8.5}m e $\omega = ${100}RPM",  -3, 4)
# polar_plot(x_axis, y_axis, case_2, 'fx_rotores', 16, 100,  f"$C_x$ (rotores) para $T = ${16}m  e $\omega = ${100}RPM"  , -3, 4)
# polar_plot(x_axis, y_axis, case_3, 'fx_rotores', 8.5, 180, f"$C_x$ (rotores) para $T = ${8.5}m e $\omega = ${180}RPM",  -3, 4)
# polar_plot(x_axis, y_axis, case_4, 'fx_rotores', 16, 180,  f"$C_x$ (rotores) para $T = ${16}m  e $\omega = ${180}RPM"  , -3, 4)
# polar_plot(x_axis, y_axis, case_1, 'fy_rotores', 8.5, 100, f"$C_y$ (rotores) para $T = ${8.5}m e $\omega = ${100}RPM",  -10, 10)
# polar_plot(x_axis, y_axis, case_2, 'fy_rotores', 16, 100,  f"$C_y$ (rotores) para $T = ${16}m  e $\omega = ${100}RPM"  , -10, 10)
# polar_plot(x_axis, y_axis, case_3, 'fy_rotores', 8.5, 180, f"$C_y$ (rotores) para $T = ${8.5}m e $\omega = ${180}RPM",  -10, 10)
# polar_plot(x_axis, y_axis, case_4, 'fy_rotores', 16, 180,  f"$C_y$ (rotores) para $T = ${16}m  e $\omega = ${180}RPM"  , -10, 10)
