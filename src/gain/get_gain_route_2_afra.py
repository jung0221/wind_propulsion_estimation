...
def _process_file(fname, leg, split_index):
    try:
        ...
        if leg == "both" or leg is None:
            ...
            mean_gain = df[gain_col].dropna().mean() * 100.0
        return {"file": fname, "timestamp": ts, "mean_gain_pct": mean_gain}
    except Exception as e:
        return {"file": fname, "error": str(e)}


def compute_trip_means(csv_dir, max_files=None, leg="both", split_index=3693, n_workers=None):
    ...
    if n_workers is None or n_workers == 1:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as exe:
            it = exe.map(_process_file, files, itertools.repeat(leg), itertools.repeat(split_index))
            for res in tqdm(it, total=len(files)):
                if res is None:
                    continue
                if isinstance(res, dict) and "error" in res:
                    print(f"[WARN] skipping {res.get('file')}: {res.get('error')}")
                    continue
                records.append(res)
...
def compute_trip_means_from_dir(csv_dir, max_files=None, leg="both", split_index=3693, n_workers=None):
    return compute_trip_means(
        csv_dir, max_files=max_files, leg=leg, split_index=split_index, n_workers=n_workers
    )
...
def plot_monthly_boxplot_dual(
    ...
    if leg == "outbound":
        ...
        title_leg = ""
    ax.set_title(f"Ganho médio mensal Aframax (otimista)")
...
def run_compare(
    ...
    df_a = compute_trip_means_from_dir(
        csv_dir_a, max_files=max_files, leg=leg, split_index=split_index, n_workers=n_workers
    )
    df_b = compute_trip_means_from_dir(
        csv_dir_b, max_files=max_files, leg=leg, split_index=split_index, n_workers=n_workers
    )
...
def _build_argparser():
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Number of worker processes for parallel file processing (default: serial)",
    )
    return p
