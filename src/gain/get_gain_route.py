...
def process_single_trip(trip_file):
    ...
    pattern = r"wind_data_year_(\d{4})_month_(\d{1,2})_day_(\d{1,2})_hour_(\d{1,2})\.csv"
...
def plot_gains_per_year(df_gain, outputfolder):
    ...
    df_100 = df_gain[['timestamp_100', 'gain_100']].dropna()
    df_100 = df_100.rename(columns={'timestamp_100': 'timestamp', 'gain_100': 'gain'})
    df_100['timestamp'] = pd.to_datetime(df_100['timestamp'])
    
    df_180 = df_gain[['timestamp_180', 'gain_180']].dropna()
    df_180 = df_180.rename(columns={'timestamp_180': 'timestamp', 'gain_180': 'gain'})
    ...
    ax.plot(monthly_gains_100.index.astype(str), 100*monthly_gains_100.values, 
            marker='o', linewidth=2.5, markersize=8, color='blue', 
            label='100 RPM', markerfacecolor='lightblue', markeredgecolor='blue',
            markeredgewidth=2)
    
    # Plot lines with markers for 180 RPM
    ax.plot(monthly_gains_180.index.astype(str), 100*monthly_gains_180.values, 
            marker='s', linewidth=2.5, markersize=8, color='red', 
            label='180 RPM', markerfacecolor='lightcoral', markeredgecolor='red',
            markeredgewidth=2)
    
    # Add horizontal dashed lines for overall means
    ax.axhline(y=overall_mean_100*100, color='blue', linestyle='--', linewidth=2, 
               alpha=0.7, label=f'Média 100 RPM: {100*overall_mean_100:.2f}')
    ax.axhline(y=overall_mean_180*100, color='red', linestyle='--', linewidth=2, 
               alpha=0.7, label=f'Média 180 RPM: {100*overall_mean_180:.2f}')
    
    # Customize the plot
    ax.set_xlabel('Mês', fontsize=14, fontweight='bold')
    ax.set_ylabel('Ganho Médio Mensal', fontsize=14, fontweight='bold')
    ax.set_title('Ganhos Médios Mensais: 100 RPM vs 180 RPM', fontsize=16, fontweight='bold')
    ...
    # # Calculate overall statistics for text box
    # overall_std_100 = df_100['gain'].std()
    # overall_std_180 = df_180['gain'].std()
    
    # # Add statistics text box
    # stats_text = (f"Estatísticas Gerais:\n"
    #              f"100 RPM - Média: {overall_mean_100*100:.4f} %\n"
    #              f"100 RPM - Desvio: {overall_std_100*100:.4f} %\n"
    #              f"180 RPM - Média: {overall_mean_180*100:.4f} %\n"
    #              f"180 RPM - Desvio: {overall_std_180*100:.4f} %\n")
    ...
    # Save the plot
    plt.savefig(os.path.join(outputfolder, 'monthly_gains_comparison.png'), 
    ...
    print(f"Ganho médio geral 100 RPM: {overall_mean_100:.4f} ({overall_mean_100*100:.2f}%)")
    print(f"Ganho médio geral 180 RPM: {overall_mean_180:.4f} ({overall_mean_180*100:.2f}%)")
    print(f"Melhoria absoluta: {overall_mean_180 - overall_mean_100:.4f}")
    print(f"Melhoria relativa: {((overall_mean_180 - overall_mean_100) / abs(overall_mean_100) * 100):.2f}%")
...
def plot_histogram(trip100, trip180, outputfolder):
    ...
    plt.savefig(os.path.join(outputfolder, 'gain_comparison_histogram.png'), dpi=300, bbox_inches='tight')
...
def calc_mean_gain_parallel_separated(csv_files, n_jobs=-1):
    print(f"[INFO] Calculating separated gains from {len(csv_files)} routes using {n_jobs} cores")
...
def save_gains_separated_to_csv(gain_per_trip100, gain_per_trip180, ship):
    ...
    gains_100_out = [item['gain_outbound'] for item in gain_per_trip100 if not np.isnan(item['gain_outbound'])]
    timestamps_100_out = [item['timestamp'] for item in gain_per_trip100 if not np.isnan(item['gain_outbound'])]
    
    gains_180_out = [item['gain_outbound'] for item in gain_per_trip180 if not np.isnan(item['gain_outbound'])]
    timestamps_180_out = [item['timestamp'] for item in gain_per_trip180 if not np.isnan(item['gain_outbound'])]
    
    # Extract return gains and timestamps
    gains_100_ret = [item['gain_return'] for item in gain_per_trip100 if not np.isnan(item['gain_return'])]
    timestamps_100_ret = [item['timestamp'] for item in gain_per_trip100 if not np.isnan(item['gain_return'])]
    
    gains_180_ret = [item['gain_return'] for item in gain_per_trip180 if not np.isnan(item['gain_return'])]
    timestamps_180_ret = [item['timestamp'] for item in gain_per_trip180 if not np.isnan(item['gain_return'])]
    ...
    print(f"[INFO] Outbound - 100 RPM: {len([x for x in gains_100_out if not np.isnan(x)])} entries")
    print(f"[INFO] Outbound - 180 RPM: {len([x for x in gains_180_out if not np.isnan(x)])} entries")
    print(f"[INFO] Return - 100 RPM: {len([x for x in gains_100_ret if not np.isnan(x)])} entries")
    print(f"[INFO] Return - 180 RPM: {len([x for x in gains_180_ret if not np.isnan(x)])} entries")
...
def plot_gains_separated(df_gains_outbound, df_gains_return, outputfolder):
    ...
    df_100_out = df_100_out.rename(columns={'timestamp_100': 'timestamp', 'gain_100': 'gain'})
    ...
    df_180_out = df_180_out.rename(columns={'timestamp_180': 'timestamp', 'gain_180': 'gain'})
    ...
    ax1.plot(monthly_gains_100_out.index.astype(str), 100*monthly_gains_100_out.values, 
            marker='o', linewidth=2.5, markersize=8, color='blue', 
            label='100 RPM', markerfacecolor='lightblue', markeredgecolor='blue')
    ax1.plot(monthly_gains_180_out.index.astype(str), 100*monthly_gains_180_out.values, 
            marker='s', linewidth=2.5, markersize=8, color='red', 
            label='180 RPM', markerfacecolor='lightcoral', markeredgecolor='red')
    
    ax1.axhline(y=df_100_out['gain'].mean()*100, color='blue', linestyle='--', linewidth=2, alpha=0.7)
    ax1.axhline(y=df_180_out['gain'].mean()*100, color='red', linestyle='--', linewidth=2, alpha=0.7)
    
    ax1.set_title('Ganhos Médios Mensais - Viagem de Ida', fontsize=14, fontweight='bold')
    ...
    df_100_ret = df_100_ret.rename(columns={'timestamp_100': 'timestamp', 'gain_100': 'gain'})
    ...
    df_180_ret = df_180_ret.rename(columns={'timestamp_180': 'timestamp', 'gain_180': 'gain'})
    ...
    ax2.plot(monthly_gains_100_ret.index.astype(str), 100*monthly_gains_100_ret.values, 
            marker='o', linewidth=2.5, markersize=8, color='blue', 
            label='100 RPM', markerfacecolor='lightblue', markeredgecolor='blue')
    ax2.plot(monthly_gains_180_ret.index.astype(str), 100*monthly_gains_180_ret.values, 
            marker='s', linewidth=2.5, markersize=8, color='red', 
            label='180 RPM', markerfacecolor='lightcoral', markeredgecolor='red')
    
    ax2.axhline(y=df_100_ret['gain'].mean()*100, color='blue', linestyle='--', linewidth=2, alpha=0.7)
    ax2.axhline(y=df_180_ret['gain'].mean()*100, color='red', linestyle='--', linewidth=2, alpha=0.7)
    
    ax2.set_title('Ganhos Médios Mensais - Viagem de Volta', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outputfolder, 'monthly_gains_separated.png'), 
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.show()
...
def save_gains_to_csv(gain_per_trip100, gain_per_trip180, outputfile):
    """
    Save gain data to CSV with columns: timestamp_100, gain_100, timestamp_180, gain_180
    
    Args:
        gain_per_trip100: List of dictionaries with 'gain' and 'timestamp' for 100 RPM
        gain_per_trip180: List of dictionaries with 'gain' and 'timestamp' for 180 RPM
        outputfile: Output CSV file path
    """
    # Extract gains and timestamps
    gains_100 = [item['gain'] for item in gain_per_trip100 if not np.isnan(item['gain'])]
    timestamps_100 = [item['timestamp'] for item in gain_per_trip100 if not np.isnan(item['gain'])]
    
    gains_180 = [item['gain'] for item in gain_per_trip180 if not np.isnan(item['gain'])]
    timestamps_180 = [item['timestamp'] for item in gain_per_trip180 if not np.isnan(item['gain'])]
    
    # Find the maximum length to pad shorter arrays with NaN
    max_length = max(len(gains_100), len(gains_180))
    
    # Pad shorter arrays with NaN/NaT
    if len(gains_100) < max_length:
        gains_100.extend([np.nan] * (max_length - len(gains_100)))
        timestamps_100.extend([pd.NaT] * (max_length - len(timestamps_100)))
    
    if len(gains_180) < max_length:
        gains_180.extend([np.nan] * (max_length - len(gains_180)))
        timestamps_180.extend([pd.NaT] * (max_length - len(timestamps_180)))
    
    # Create DataFrame
    df_gains = pd.DataFrame({
        'timestamp_100': timestamps_100,
        'gain_100': gains_100,
        'timestamp_180': timestamps_180,
        'gain_180': gains_180
    })
    ...
    print(f"[INFO] Gains with timestamps saved to {outputfile}")
...
def main():
    ...
    parser.add_argument("--separated", action="store_true", help="Process separated outbound/return gains")
    ...
    if args.separated:
        ...
        if not os.path.exists(outbound_file) or not os.path.exists(return_file):
            gain_per_trip100 = calc_mean_gain_parallel_separated(csv_files_100)
            gain_per_trip180 = calc_mean_gain_parallel_separated(csv_files_180)
            save_gains_separated_to_csv(gain_per_trip100, gain_per_trip180, ship)
        ...
        if args.total_gain: 
            ...
            print(f"[INFO] Total gain: {final_gain}")
        
        if args.gain_histograms:
            plot_gains_per_year(df_gains, f"../{ship}")
            plot_histogram(gain_per_trip100, gain_per_trip180, f"../{ship}")
